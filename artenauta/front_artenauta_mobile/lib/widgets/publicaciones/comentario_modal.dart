...
class _ComentariosModalState extends State<ComentariosModal> {
  ...
  Future<void> _cargarComentarios() async {
    try {
      final comentarios = await _comentariosService.obtenerComentarios(
  ...
  Future<void> _publicarComentario(String contenido) async {
    ...
    if (_idUsuario == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('No se pudo identificar al usuario activo.'),
  ...
  Future<void> _eliminarComentario(int idComentario) async {
    try {
      await _comentariosService.eliminarComentario(idComentario);
      await _cargarComentarios();
    } catch (e) {
      if (!mounted) return;
