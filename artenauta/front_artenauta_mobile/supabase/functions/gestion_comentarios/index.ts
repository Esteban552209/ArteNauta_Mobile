import { serve } from "https://deno.land/std@0.168.0/http/server.ts"
import { createClient } from "https://esm.sh/@supabase/supabase-js@2"
import { verificarToken } from "../_shared/verificar_token.ts"

const corsHeaders = {
  'Access-Control-Allow-Origin': '*',
  'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type',
  'Access-Control-Allow-Methods': 'GET, DELETE, OPTIONS',
}

serve(async (req) => {
  if (req.method === 'OPTIONS') return new Response('ok', { headers: corsHeaders })

  try {
    const usuarioAuth = verificarToken(req) as any;
    if (usuarioAuth.id_rol !== 3) throw new Error('Acceso denegado: Se requieren permisos de administrador');

    const supabase = createClient(Deno.env.get('SUPABASE_URL') ?? '', Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? '')

    // MÉTODO GET: OBTENER TODOS LOS COMENTARIOS
    if (req.method === 'GET') {
      const { data, error } = await supabase
        .from("comentarios")
        .select(`
            *,
            usuarios (
                id_usuario,
                nombre,
                email
            ),
            publicaciones (
                id_publicacion,
                titulo
            )
        `)
        .order("fecha_comentario", { ascending: false });

      if (error) throw error;
      return new Response(JSON.stringify(data || []), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }

    // MÉTODO DELETE: ELIMINAR COMENTARIO
    if (req.method === 'DELETE') {
      const url = new URL(req.url);
      const id_comentario = url.searchParams.get("id_comentario");

      if (!id_comentario) throw new Error("Falta el id del comentario");

      const { error } = await supabase
        .from("comentarios")
        .delete()
        .eq("id_comentario", id_comentario);

      if (error) throw error;
      return new Response(JSON.stringify({ mensaje: "Comentario eliminado con éxito" }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }

    throw new Error('Método no soportado');

  } catch (error: any) {
    return new Response(JSON.stringify({ error: error.message }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  }
})
